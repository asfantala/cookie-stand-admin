      ...
      {reports.length === 0 ? (
        ...
        <table className="w-full border-collapse border border-green-500">
          ...
          <tbody>
            {reports.map((report) => (
              <tr key={report.id}>
                <td className="relative border border-green-500">
                  <span>{report.location}</span>
                  <button
                    className="absolute inline-flex items-center justify-center h-6 w-6 text-red-500 right-2 top-1/2 transform -translate-y-1/2"
          ...
          <tfoot>
            <tr>
              <td className="bg-green-500 text-white text-left px-4 py-2 border border-green-500">
                Total
              </td>
              {hours.map((_, index) => (
                <td
